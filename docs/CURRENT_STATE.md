...
- Background workers stream honest named stages through the existing job poll response: preparing the light curve, searching periods with TLS/BLS, building the transit model and plots, calculating diagnostics, and finalizing results. Run Status displays the current stage alongside its independent elapsed timer.
